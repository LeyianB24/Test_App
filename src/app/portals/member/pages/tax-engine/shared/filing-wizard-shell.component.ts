...
@Component({
  ...
  template: `
    <div class="wizard-shell animate-up max-w-5xl mx-auto p-6">
      <header class="wizard-header mb-10 text-center">
        <h1 class="text-4xl font-black text-slate-800 tracking-tight mb-2">{{ title() }}</h1>
      ...
      <!-- Actions -->
      <footer class="wizard-actions flex justify-between items-center bg-white p-4 rounded-2xl border border-slate-100 shadow-sm">
        <button 
          class="btn-back" 
          (click)="back.emit()" 
  ...
  styles: [`
    ...
    .step-blob {
        ...
        background: white; border: 2.5px solid #E2E8F0; color: #94A3B8;
        font-weight: 900; font-size: 1.1rem; transition: 0.4s cubic-bezier(0.175, 0.885, 0.32, 1.275);
    }
    .step-node.active .step-blob { border-color: #E31E24; color: #E31E24; transform: scale(1.1); box-shadow: 0 0 0 6px rgba(227, 30, 36, 0.1); }
    .step-node.done .step-blob { background: #10B981; border-color: #10B981; color: white; }
    
    .step-title { margin-top: 12px; font-size: 0.75rem; font-weight: 900; color: #94A3B8; text-transform: uppercase; letter-spacing: 1px; }
    .step-node.active .step-title { color: #E31E24; }
    .step-node.done .step-title { color: #10B981; }

    .step-line { flex-grow: 1; max-width: 150px; height: 4px; background: #E2E8F0; margin-bottom: 30px; border-radius: 2px; position: relative; }
    .step-line.filled { background: #10B981; }

    .btn-continue, .btn-submit {
        background: linear-gradient(135deg, #E31E24, #B91C1C); color: white;
    ...
    .btn-back {
        background: white; color: #64748B; padding: 14px 24px; border-radius: 16px; 
        font-weight: 800; border: 2px solid #F1F5F9; display: flex; align-items: center; cursor: pointer; transition: 0.2s;
    }
    .btn-back:hover:not(:disabled) { border-color: #E2E8F0; background: #F8FAF7; color: #334155; }
