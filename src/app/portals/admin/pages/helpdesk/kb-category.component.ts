      ...
      @if (category(); as cat) {
        ...
        <div class="grid grid-cols-1 gap-6">
          @for (art of articles(); track art.id) {
            <div 
              class="content-card-premium p-8 hover:border-accent/30 transition-all cursor-pointer group flex justify-between items-center"
              [routerLink]="['/helpdesk/knowledge-base/article', art.slug]"
            >
              <div class="flex-1">
                <h3 class="text-2xl font-black text-primary group-hover:text-accent transition-colors mb-3">{{ art.title }}</h3>
                <div class="flex items-center gap-6 text-[10px] font-black uppercase tracking-widest text-tertiary">
              ...
              <div class="w-10 h-10 rounded-full bg-surface-2 flex items-center justify-center group-hover:bg-accent group-hover:text-white transition-all transform group-hover:translate-x-1">
                <svg width="20" height="20" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-width="3" d="M9 5l7 7-7 7"/></svg>
              </div>
            </div>
          } @empty {
            <div class="py-20 text-center opacity-50 bg-surface-1 rounded-3xl border-2 border-dashed border-default">
              <p class="text-xs font-black uppercase tracking-[0.2em] text-tertiary italic">No protocols found in this sector</p>
            </div>
          }
        </div>
      } @else {
        <div class="flex flex-col items-center justify-center py-40">
           ...
           <p class="text-[10px] font-black uppercase tracking-widest text-tertiary">Synchronizing Sector Data...</p>
        </div>
      }
    </div>
  `,
