  ...
  template: `
    <header class="topbar-precision">
      <div class="breadcrumb-container-precision">
        ...
        <a [routerLink]="'/admin-portal/dashboard'" class="breadcrumb-parent">ADMIN CONSOLE</a>
        ...
        <div class="relative">
          ...
          <div class="notifications-panel-precision" *ngIf="isNotificationsOpen()" (click)="$event.stopPropagation()">
            <div class="notifications-header-precision">
              <span class="notifications-title-precision">System alerts</span>
              <a href="javascript:void(0)" class="mark-read-link-precision" (click)="markAllRead()">Archive all</a>
            </div>
            
            <div class="custom-scrollbar overflow-y-auto max-h-[380px]">
              <div *ngIf="notifications().length === 0" class="p-8 text-center color-black-500 text-xs text-uppercase tracking-widest">
                All systems active.
              </div>
              @for (note of notifications(); track note.id) {
                <div class="notification-item-precision" [class.unread]="!note.read" (click)="markAsRead(note)">
                  <div class="notif-icon-box-precision info">
        ...
        <div class="relative">
          ...
          <div class="user-menu-elite pointer-events-auto" *ngIf="isMenuOpen()" (click)="$event.stopPropagation()">
              <ul class="menu-links-elite">
                <li><a [routerLink]="'/admin-portal/profile'"><span class="nav-link-label">Staff Profile</span></a></li>
                <li><a [routerLink]="'/admin-portal/settings'"><span class="nav-link-label">Global Settings</span></a></li>
                <li class="m-divider"></li>
                <li><button (click)="logout.emit()" class="m-logout-btn font-bold"><span class="nav-link-label">Seal Session</span></button></li>
              </ul>
          </div>
        </div>
      </div>
